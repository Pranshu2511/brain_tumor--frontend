...
const Prediction = () => {
  ...
  const refreshCaptcha = () => {
    setUserCaptcha("");
    setCaptchaImg(`https://backend-brain-9q15.onrender.com/generate-captcha?${Date.now()}`);
    setShowCaptcha(true);
    setOtpSent(false);
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch("https://backend-brain-9q15.onrender.com/predict", {
        method: "POST",
        body: formData,
      });
  ...
  const handleCaptchaSubmit = async () => {
    setVerifying(true);
    try {
      const res = await fetch("https://backend-brain-9q15.onrender.com/verify-captcha", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        const otpRes = await fetch("https://backend-brain-9q15.onrender.com/send-otp", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleOtpSubmit = async () => {
    setVerifying(true);
    try {
      const res = await fetch("https://backend-brain-9q15.onrender.com/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
